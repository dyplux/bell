...
`;
  ...
  function renderDecisionStory() {
    ...
    const hint = alert.state === 'do_not_compare' ? 'clear the contradiction first' : alert.state === 'investigate' ? 'classify before shortlist' : Number(alert.token_count || 0) === 1 ? 'single representation · verify externally' : 'facts only · continue external diligence';
